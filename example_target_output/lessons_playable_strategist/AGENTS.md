...
Core job: Choose the playable corridor that can honestly express the section's teaching burden against current product readback.
...
Immediate local read: the Section Lesson Map Contract in SECTION_LESSON_MAP.md at `section_root/_authoring/SECTION_LESSON_MAP.md`, the section promise, any explicit user playable guidance, and the current product readback before you rank options.
...
Choose the playable corridor that can honestly express the section's teaching burden against current product readback.

- Restate the section's real teaching job before you look at playables.
- Read the current attached-checkout files or current product captures for the candidate playable families before you rank options.
...
- For every serious option, say what teaching job it supports, what it makes easier or harder downstream, and why it won or lost.
- Keep the ranked options, rejected options, and downstream rules in SECTION_PLAYABLE_STRATEGY.md itself unless a modeled support file truly owns part of that proof.
...
- Keep the accepted playable contract plain enough that downstream lanes do not have to guess what you meant.
...
- This file must include
  - The accepted playable family and why it fits the section's teaching job.
...
- This packet owns the accepted playable families, rejected playable families, and the downstream constraints later lesson lanes must preserve.
- Keep the ranked options, why each rejected option lost, and the current product readback inside this file unless a modeled support file truly owns them.
- The next role should be able to see the real choice set and what the winner requires downstream without hunting for side notes.
...
- Use these inputs
  ...
  - explicit user playable guidance and the current product readback when they matter
- This role produces
  - SECTION_PLAYABLE_STRATEGY.md at `section_root/_authoring/SECTION_PLAYABLE_STRATEGY.md` with the current product readback, ranked options, and downstream rules
- Next owner if accepted
  - Lessons Acceptance Critic
- Stop here if
  - Stop when the accepted playable families, rejected families, and downstream constraints are explicit enough for critic review.

<a id="use-this-role-when"></a>
## When To Use This Role

- Use this role when a section needs an honest playable corridor and capstone choice.
...
- Expect this lane to stop with Section Playable Strategy Contract ready in SECTION_PLAYABLE_STRATEGY.md at `section_root/_authoring/SECTION_PLAYABLE_STRATEGY.md` for critic review.
