...
| `151_stage_basics` | Top-level `stage Name: "Title"` declarations bind an owner skill to typed inputs, optional artifacts, an optional emitted receipt, `applies_to:` flow membership, an advance condition, and a closed `checkpoint:` value. Stage shape problems fail with `E559`; graph compile also checks reached-stage `applies_to:` coverage with `E562`. |
