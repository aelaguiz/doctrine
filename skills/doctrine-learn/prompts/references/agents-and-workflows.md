...
- A concrete agent may declare a title after the key: `agent Writer: "Writer"`.
  Bare readable refs default to that title. `{{Ref:key}}` keeps the structural
  name.
...
surface for bare refs. Use `{{AgentRef:key}}` or `{{AgentRef:name}}` when you
need the structural name instead.

- bare `{{AgentRef}}` defaults to the title when one exists.
- `{{AgentRef:title}}` resolves the visible title explicitly.
- `{{AgentRef:key}}` resolves the declaration key.
