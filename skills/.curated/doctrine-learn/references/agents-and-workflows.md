...
Skills own capabilities; agents reference them. If the role prose is telling the
turn how to call a script, run a tool, or meet a hard requirement, that content
belongs on the `skill`, not on the agent. Violations fail loud under
`agent-linter` finding `AL250`.
