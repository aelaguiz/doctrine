...
## Bare And Named Readable Blocks

Every readable block kind has two forms. The bare form drops the CNAME key and any heading string. The named form keeps both. The bare form is the first-class default when the block is just body prose inside a section. The named form is for blocks another declaration must point at, or blocks that carry `required`, `advisory`, `optional`, or `when` metadata.

Bare form looks like this:
...
section summary: "Summary"
    ...
    footnotes:
        cite: "Referenced in the citation."
...
"""
    ...
    markdown:
        text: """Raw markdown line.
"""
```

Named form adds a CNAME and a heading string. Use named form when another declaration must reference the block, when the block needs `required`, `advisory`, or `when` metadata, or when the author wants a visible H3 heading:

```prompt-fragment
definitions done_when: "Done When" required
    verdict: "Verdict"
        "Say whether the release is ready."
...
See `examples/128_readable_block_parity/` for every bare form side by side.
...
The use site inherits every column. Addressable refs still work — `ReleaseGates:columns.evidence.title` and `ReleaseGuide:release_gates.rows.package_smoke` both resolve when used inside interpolation.
...
Agents can still address the nested blocks through a `RootReadableGuide:evidence_note.title` interpolation. See `examples/114_workflow_root_readable_blocks/`.
...
A triple-quoted body may contain two quotes on its own, but a bare `"""` ends the literal. Escape the first quote as `\"""` to embed a literal `"""` sequence. This is common when the prose shows a Python docstring or a prompt snippet inside its own prose.

```prompt
document EscapeDemo: "Triple Quote Escape Demo"
    markdown python_snippet: "Python Snippet" advisory
        text: """
Python docstring example:

\"\"\"
Return the shipped proof link.
\"\"\"
"""
```

See `examples/125_multiline_escape_triple_quote/` for the shipped proof.
