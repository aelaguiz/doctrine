...
const INTERPOLATION_EXPR_RE = new RegExp(
  `^(${DOTTED_NAME_PATTERN})(?::${DOTTED_NAME_PATTERN})?$`,
);
...
function collectInterpolationSites(lineText, lineNumber) {
  ...
  for (const match of lineText.matchAll(INTERPOLATION_RE)) {
    ...
    const rawRef = expressionMatch[1];
    const ref = parseNameRef(rawRef);
    if (!ref) {
      continue;
    }

    const startCharacter = (match.index ?? 0) + 2;
    sites.push({
      type: "readableDeclRef",
...
function getRecordChildBodySpec(lineText, lineNumber, fieldKind) {
  if (
    new RegExp(
      `^\\s*(?:${IDENTIFIER_PATTERN}\\s*:\\s*(?:${STRING_PATTERN}|${DOTTED_NAME_PATTERN})|${DOTTED_NAME_PATTERN})\\s*$`,
    ).test(lineText)
  ) {
    return {
      type: "record_body",
...
function createFirstMatchRange(lineText, lineNumber, rawValue) {
  return createRangeFromStart(
    lineText,
    lineNumber,
    lineText.indexOf(rawValue),
    rawValue.length,
  );
}
...
function createLastMatchRange(lineText, lineNumber, rawValue) {
  return createRangeFromStart(
    lineText,
    lineNumber,
    lineText.lastIndexOf(rawValue),
    rawValue.length,
  );
}

function createRangeFromStart(lineText, lineNumber, startCharacter, length) {
