...
| `E556` | Invalid tracked source path | A `source.track:` path is absolute, uses `..`, uses backslashes, leaves the receipt source root, or names a generated cache artifact such as `__pycache__`, `.pytest_cache`, `.pyc`, or `.pyo`. |
...
| `E566` | Invalid skill relation | A `relations:` entry on a top-level `skill` has an unknown relation kind, points at a missing skill, or omits `why:` while a graph requires `relation_reason`. |
