...
- Added the new flagship example at `examples/68_flow_visualizer_showcase/`, registered `example_68_flow_visualizer_showcase` in `pyproject.toml`, generated checked `build_ref/` docs plus `AGENTS.flow.{d2,svg}`, and embedded the checked SVG in `README.md`.
...
- `uv run --locked python -m doctrine.emit_docs --target example_68_flow_visualizer_showcase`
