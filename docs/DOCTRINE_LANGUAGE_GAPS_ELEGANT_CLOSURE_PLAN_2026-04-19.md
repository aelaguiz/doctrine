...
Completed work:
- Grammar: `doctrine/grammars/doctrine.lark` `output_target_body` now accepts a sibling `output_target_typed_as_stmt` production (`typed_as: <name_ref>`) alongside the existing `delivery_skill` and record items.
- Model: `doctrine/_model/io.py` `OutputTargetDecl` gains a frozen `typed_as: NameRef | None` field. The new `OutputTargetTypedAsPart` helper in `doctrine/_parser/parts.py` carries the parsed ref through the transformer.
- Parser: `doctrine/_parser/io.py` consumes `output_target_typed_as_stmt` and wires the `typed_as` body line into `OutputTargetDecl`. Duplicate `typed_as:` lines fail loud at parse time.
- Validator: `doctrine/_compiler/resolve/outputs.py` now calls `_resolve_output_target_typed_as` when building a `ResolvedOutputTargetSpec` — the resolver raises E533 when `typed_as:` names an entity that is neither a `document`, `schema`, nor `table`. `doctrine/_compiler/compile/outputs.py` runs a new `_validate_output_target_downstream_family_match` step that compares a downstream output's `structure:` / `schema:` family against the target's `typed_as:` family and raises E534 on mismatch. The matching `ResolvedOutputTargetTypedAs` + extended `ResolvedOutputTargetSpec` live in `doctrine/_compiler/resolved_types.py`.
- Contract emission: the emitted `## Outputs` contract table now carries a `Typed As` row directly after the `Target` row when the underlying target declares `typed_as:`, wired through the existing `doctrine/_compiler/compile/outputs.py` contract-row assembly.
- Example: `examples/143_typed_handoff_note_identity/` ships with a positive `render_contract` case proving the `Typed As` row plus two `compile_fail` cases for E533 (typed_as -> enum) and E534 (downstream `structure:` family mismatch). The manifest uses the shipped `exception_type = "CompileError"`, `error_code`, `location_line` shape.
- Diagnostics: `doctrine/_diagnostic_smoke/fixtures_authored.py` adds `_output_target_typed_as_unsupported_kind_source` and `_output_target_typed_as_family_mismatch_source`; `doctrine/_diagnostic_smoke/fixtures.py` re-exports them; `doctrine/_diagnostic_smoke/compile_checks.py` registers `_check_output_target_typed_as_unsupported_kind_emits_e533` and `_check_output_target_typed_as_family_mismatch_emits_e534` in the smoke runner.
- Docs: `docs/LANGUAGE_REFERENCE.md` adds the `typed_as:` bullet plus a typed handoff-note example and the emitted contract table excerpt. `docs/COMPILER_ERRORS.md` registers `E533` and `E534` between the existing `E532` and `E535` rows.
- Curated teaching: `skills/doctrine-learn/prompts/refs/outputs_and_schemas.prompt` adds a `Typed Handoff-Note Targets` section. Both `skills/.curated/doctrine-learn/` and `skills/doctrine-learn/build/` mirrors were rebuilt via the `doctrine_learn_skill` and `doctrine_learn_public_skill` emit targets.
- VSCode: `editors/vscode/syntaxes/doctrine.tmLanguage.json` adds `typed_as` to the second-wave keyword list so the keyword highlights inside `output target` bodies; `editors/vscode/resolver.js` extends the keyed-decl ref regex with `typed_as` and resolves the ref to any of `document`, `schema`, or `table` declarations for go-to-definition.
- Version bump: `docs/VERSIONING.md` advances the current Doctrine language version from 5.2 to 5.3 with a plain-language additive-move entry; `CHANGELOG.md` extends the Unreleased block (4.0 → 5.3), logs the matching `Added` bullets, and refreshes the verification checklist.
- Verification: `make verify-examples` green on 420 cases (was 417 before this phase); `make verify-diagnostics` green including the two new E533 / E534 smoke checks.

Decision Log:
- Example number drift: the plan named the shipped example as `142_typed_handoff_note_identity`, but Phase 3 had already claimed `142_skill_host_receipt_envelope`. The typed-handoff example landed at `143_typed_handoff_note_identity` to keep the numbered corpus monotonic; the plan checklist, exit criteria, and CHANGELOG reference the corrected `143_*` path.
- E534 scope: the downstream family mismatch check compares the output's own `structure:` or `schema:` against the target's `typed_as:` family — a local, declaration-scoped check — rather than threading through cross-agent input-binding analysis. The local check keeps validation tractable and still catches the drift mode the plan named.
...
Completed work:
- Grammar: `doctrine/grammars/doctrine.lark` now lets `skill_entry_body` accept the shipped expr-based `mode_stmt` production verbatim (no new production; reused across review cases, law matchers, and output-shape selectors). `output_shape_selector_stmt` gains a new expr-based alternation alongside the legacy enum-only form.
- Model: `doctrine/_model/io.py` adds `SkillEntry.mode: ModeStmt | None`; the `ResolvedSkillEntry` companion in `doctrine/_compiler/resolved_types.py` carries the resolved mode through to emission. `OutputShapeSelectorConfig` now carries an optional expr field so selectors can reach the expr-based canonical form.
- Parser: `doctrine/_parser/skills.py` consumes the new `mode_stmt` alternation on skill entries and populates `SkillEntry.mode`. `doctrine/_parser/io.py` consumes both selector forms and populates the new optional expr field.
- Validator: `doctrine/_compiler/validate/agents.py` adds `_validate_skill_entry_mode` — it resolves the mode `as` target to a declared enum, rejects audit-mode bindings that emit to output targets, and verifies that the bound skill package declares a contract variant per mode. `doctrine/_compiler/compile/output_selectors.py` emits `E543` when an output-shape selector still uses the enum-only form. The resolver in `doctrine/_compiler/resolve/skills.py` now threads the resolved mode through to the compiled skill entry.
- Diagnostics: `doctrine/_diagnostic_smoke/fixtures_authored.py` adds fixture sources for E540/E541/E542/E543; `doctrine/_diagnostic_smoke/fixtures.py` re-exports them; `doctrine/_diagnostic_smoke/compile_checks.py` registers the four new smoke checks with verified line numbers (E540=22, E541=33, E542=22, E543=19).
- Example: `examples/144_skill_binding_producer_audit_mode/` ships with two `render_contract` cases (producer agent + audit agent sharing one `SharedSkill` package) and four `compile_fail` cases covering E540 (mode enum ref unresolved), E541 (audit mode + output emission), E542 (mode CNAME not in enum), and E543 (deprecated enum-only selector form).
- Migration: `examples/138_output_shape_case_selector/` and every nested case directory (`cross_flow_enum_identity/`, `inherited_parent/`, `invalid_*`) migrate to the expr-based `mode CNAME = EnumName.case as EnumName` form. One enum-only regression fixture is preserved under `examples/144_skill_binding_producer_audit_mode/prompts/invalid_enum_only_selector/` to keep the E543 soft-deprecation signal honest until the timebox expires.
- Docs: `docs/LANGUAGE_REFERENCE.md` adds a "Skill Binding Mode" subsection under the Skill Packages section with the producer/audit idiom and rules; replaces the output-shape selector description with the expr-based canonical form plus an E543 soft-deprecation note. `docs/COMPILER_ERRORS.md` registers `E540`, `E541`, `E542`, and `E543` with plain-language fixes.
- Curated teaching: `skills/doctrine-learn/prompts/refs/skills_and_packages.prompt` adds a `section skill_binding_mode` block with the producer/audit idiom and cross-reference; `skills/doctrine-learn/prompts/refs/outputs_and_schemas.prompt` adds a `section output_shape_selectors` block teaching the expr-based canonical form with the E543 note. Both `skills/.curated/doctrine-learn/` and `skills/doctrine-learn/build/` mirrors were rebuilt via the `doctrine_learn_skill` and `doctrine_learn_public_skill` emit targets.
- Agent-linter: `skills/agent-linter/prompts/refs/finding_catalog.prompt` adds two new findings (`AL970` "Critic Re-Invokes Producer Skill Instead Of Binding Audit Mode" and `AL980` "Output-Shape Selector Uses Deprecated Enum-Only Mode Form") in the summary table plus full finding bodies in the `full_calibration` section. Both `skills/.curated/agent-linter/` and `skills/agent-linter/build/` mirrors were rebuilt via the `doctrine_agent_linter_skill` and `doctrine_agent_linter_public_skill` emit targets.
- Version bump: `docs/VERSIONING.md` advances the current Doctrine language version from 5.3 to 5.4 with a plain-language additive-move entry that names the E543 timebox; `CHANGELOG.md` extends the Unreleased block (now 4.0 → 5.4), logs the matching `Added` bullets, and introduces a new `### Deprecated (language 5.3 -> 5.4)` section recording the enum-only output-shape selector soft-deprecation.
- Decision Log: Section 10 gains a new entry `## 2026-04-19 - Normalize output_shape_selector_stmt to expr-based mode form (soft-deprecation with timebox)` recording context, options, decision, the E543-as-fatal architectural call (Doctrine's compiler has no warning channel, so the soft-deprecation is implemented as a fatal compile error until the grammar removes the form at the next minor bump), consequences, and the timebox.
- Verification: `make verify-examples` green on 426 cases; `make verify-diagnostics` green including all four new smoke checks.

Decision Log:
- E543 as fatal, not warning: Doctrine's compiler has no warning infrastructure — diagnostics are either silent or fatal. Rather than invent a warning channel for one soft-deprecation, E543 is implemented as a fatal compile error with an authored migration hint ("Use the expr-based `mode CNAME = expr as name_ref` form"). The enum-only grammar alternative stays parseable so existing corpora still reach the validator and get the clear message; the alternative is scheduled for removal at the next minor bump. See §10 Decision Log entry.
- Exit-criterion softening: the plan's exit criteria called the E543 diagnostic "non-fatal; compilation continues." The shipped behavior is fatal for the reason above. The §10 Decision Log entry records this deliberate deviation rather than weakening the requirement silently.
- VSCode build: `cd editors/vscode && make` is failing on pre-existing parallel WIP in `editors/vscode/resolver.js` (unrelated `testImportLinks` assertion on `simple.greeting`), not on Phase 5 syntax changes. Phase 5 added no new syntax tokens (the `mode` keyword was already highlighted), so no tmLanguage delta was required. Flagged in the worklog.
...
Completed work:
- Grammar extended at `doctrine/grammars/doctrine.lark` `agent_slot_abstract` to accept `abstract CNAME [: name_ref]`. Parser wiring in `doctrine/_parser/agents.py` populates `declared_type: NameRef | None` on `AuthoredSlotAbstract` (model moved from `AuthoredSlotField` to the actual abstract-slot dataclass).
- Model field added in `doctrine/_model/agent.py`: `AuthoredSlotAbstract.declared_type: NameRef | None`.
- Validator `_validate_typed_abstract_slot_binding` added to `ValidateAgentsMixin` in `doctrine/_compiler/validate/agents.py`. It runs ahead of slot resolution so it fires before E299/E901. Emits `E538` (concrete binds typed abstract slot to wrong-family entity or with inline block) and `E539` (type annotation references unknown entity).
- Resolver hardened in `doctrine/_compiler/resolve/agent_slots.py`: typed-abstract concrete bindings produce a new `ResolvedTypedAgentSlot` variant defined in `doctrine/_compiler/resolved_types.py`. The compile path in `doctrine/_compiler/compile/agent.py` skips typed slots from `field_specs`, so typed bindings are purely compile-time-checked and do not render in the concrete agent's output surface.
- Diagnostic smoke fixtures added in `doctrine/_diagnostic_smoke/fixtures_authored.py`, registered in `fixtures.py`, and exercised from `compile_checks.py` (`_check_concrete_agent_wrong_family_binding_emits_e538`, `_check_abstract_slot_annotation_unresolved_emits_e539`). `make verify-diagnostics` green.
- Example shipped at `examples/145_abstract_agent_typed_parameters/` (next free slot; the Phase 6 checklist originally named slot 144 but slot 144 already belongs to `144_skill_binding_producer_audit_mode`). `cases.toml` carries two render_contract cases (abstract + concrete, concrete agents alone) and two compile_fail cases (`invalid_wrong_family_binding` → E538, `invalid_unresolved_annotation` → E539). All four cases pass.
- Docs updated: `docs/LANGUAGE_REFERENCE.md` adds the `abstract <slot_key>: <TypedEntityRef>` bullet with family narrowing, E538/E539 enforcement note, and the non-convergence cross-reference. `docs/COMPILER_ERRORS.md` adds E538/E539 rows between E537 and E540.
- Curated skills updated at their `.prompt` sources and regenerated: `skills/doctrine-learn/prompts/refs/agents_and_workflows.prompt` grew a new inheritance bullet teaching the typed annotation; `skills/agent-linter/prompts/refs/finding_catalog.prompt` gained `AL285 Near-Duplicate Agents Differ Only By Typed Entity Reference` in both the table and the full-calibration section. Both `skills/.curated/...` mirrors regenerated via `uv run --locked python -m doctrine.emit_skill --target doctrine_learn_public_skill` and `--target doctrine_agent_linter_public_skill`.
- VSCode syntax: `editors/vscode/syntaxes/doctrine.tmLanguage.json` extended so the existing `abstractField` pattern matches the optional `: <TypedEntityRef>` annotation with captures for the `:` punctuation and the type name.
- `CHANGELOG.md` language-version line bumped `4.0 -> 5.5`, and a new 5.4 → 5.5 bullet describes the typed abstract-slot annotation, E538/E539, and example 145. `docs/VERSIONING.md` bumped "Current Doctrine language version: 5.4" → "5.5" and added a 5.4 → 5.5 additive-moves paragraph.
- Decision Log: appended `## 2026-04-19 - Typed abstract-agent parameters stay narrow; no convergence with skill host_contract slots or output-schema fields` to Section 10 with the full context/options/decision/consequences/follow-ups structure.
- `make verify-examples` green on 01-145. `make verify-diagnostics` green including the new E538/E539 fixtures.
...
Completed work:
- Grammar: `doctrine/grammars/doctrine.lark` gains a top-level `rule_decl` production with closed `rule_scope_block` and `rule_assertions_block` bodies and `message:` trailer. Predicates are limited to the shipped closed set: `agent_tag: <CNAME>`, `flow: <NameRef>`, `role_class: <CNAME>`, `file_tree: <STRING>`; assertions are limited to `requires inherit <NameRef>`, `forbids bind <NameRef>`, and `requires declare <CNAME>`.
- Model: `doctrine/_model/rule.py` defines `RuleDecl`, `RuleScope`, `RuleAssertion`, the four predicate dataclasses (`AgentTagPredicate`, `FlowPredicate`, `RoleClassPredicate`, `FileTreePredicate`), and the three assertion dataclasses (`RequiresInheritAssertion`, `ForbidsBindAssertion`, `RequiresDeclareAssertion`). `doctrine/model.py` re-exports `RuleDecl`; `doctrine/_model/declarations.py` adds `RuleDecl` to the `Declaration` type alias.
- Parser: wiring emits `RuleDecl` directly from the new grammar production.
- Indexing: `doctrine/_compiler/indexing.py` registers `rules_by_name` on both `UnitDeclarations` and `IndexedFlow`; `doctrine/_compiler/declaration_kinds.py` adds a `rule` `DeclarationKind` entry with `readable=False`, `addressable_root=False`, `registry_attr="rules_by_name"`.
- Validator: `doctrine/_compiler/validate/rules.py` ships `ValidateRulesMixin` and is composed as the last base of `ValidateMixin` at `doctrine/_compiler/validate/__init__.py`. `compile_agent_from_unit` invokes `_validate_all_rules_in_flow(flow)` with a per-flow-identity cache (`_rules_validated_flows`) so rule evaluation runs once per flow even when multiple concrete agents in the flow compile. Scope predicates combine with OR semantics. Ancestor-chain walking reuses `_resolve_parent_agent_decl` with a cycle guard; slot-declare checks walk `AuthoredSlotField | AuthoredSlotAbstract | AuthoredSlotInherit | AuthoredSlotOverride` across the declared ancestor chain.
- Diagnostics: `RULE001` (unknown scope-predicate target), `RULE002` (unknown assertion target), `RULE003` (scoped agent fails `requires inherit`), `RULE004` (scoped agent violates `forbids bind`), `RULE005` (scoped agent fails `requires declare`) all emit through the shared `compile_error` path. The public formatter at `doctrine/diagnostics.py` is already code-agnostic and renders `RULE###` codes identically to `E###` codes; no per-code branch was added.
- Diagnostic smoke: `doctrine/_diagnostic_smoke/fixtures_authored.py` adds five rule fixture sources; `fixtures.py` re-exports them; `compile_checks.py` registers five new smoke checks (`_check_rule_unknown_scope_target_emits_rule001` through `_check_rule_requires_declare_violated_emits_rule005`). `make verify-diagnostics` green with the new RULE001-RULE005 fixtures.
- Example: `examples/146_declarative_project_lint_rule/` ships one happy-path `render_contract` rule (`role_class: Composer` + `requires inherit UpstreamPoisoningInvariant`) plus one `compile_fail` case per shipped `RULE###` code. All seven cases pass under `make verify-examples`.
- Docs: `docs/LANGUAGE_REFERENCE.md` adds a new `## Project Rules` section covering declaration shape, the closed scope/assertion predicate sets, evaluation semantics, and a cross-reference to example 146. `docs/COMPILER_ERRORS.md` adds a new `### Rule-check codes` subsection inside `## Current Stable Codes` with RULE001-RULE005 rows plus the closed-predicate stability note (`RULE006-RULE099` reserved for closed-predicate extensions; `RULE100+` reserved for any future open-expression-language evolution).
- Curated teaching: `skills/.curated/doctrine-learn/references/rules.md` is the new teaching reference, registered in `skills/.curated/doctrine-learn/SKILL.md`'s reference map between `reviews.md` and the other entity references. `skills/.curated/agent-linter/references/finding-catalog.md` adds `AL990 Project Lacks An Enforcement Rule For Shared Inheritance Invariant` with What/Why/Default fix/Good/Bad/See-also sections.
- VSCode: `editors/vscode/syntaxes/doctrine.tmLanguage.json` registers a `ruleDeclaration` pattern for top-level `rule <Name>:` lines plus a `ruleKeyword` repo entry that highlights the closed keyword set `scope|assertions|requires|forbids|agent_tag|role_class|file_tree|declare|message`. Both are included in the top-level patterns array alongside the existing declaration includes.
- Version bump: `CHANGELOG.md` advances the Unreleased block's `Language version` line to `4.0 -> 5.6` and adds a 5.5 → 5.6 additive-moves paragraph plus a new `### Added` bullet cluster for the rule primitive, example 146, curated rules reference, and AL990. `docs/VERSIONING.md` advances "Current Doctrine language version: 5.5" → "5.6" and adds a 5.5 → 5.6 additive-moves paragraph naming the closed-predicate `scope:` set, the closed-predicate `assertions:` set, the shipped RULE001-RULE005 codes, and the `RULE###`-band stability rule.
- Decision Log: Section 10 gains a new entry `## 2026-04-20 - Rule primitive ships with a closed predicate surface and its own RULE### diagnostic band` (see Section 10 below).
- Parallel work note: the live repo tree carries unrelated in-progress work from the user on several adjacent files (`output_selectors.py`, `compile/agent.py`, several model/parser modules, examples 138/139/140, plus the VSCode `resolver.js`). Per the standing `auto-implement` directive ("I'm doing stuff in parallel don't freak out about it"), this phase did not touch that in-progress surface; P7 changes are confined to the rule primitive's own files. `cd editors/vscode && make` reports the same pre-existing `testImportLinks` / `simple.greeting` failure as Phase 5's worklog note — that is a `resolver.js` link-provider check unrelated to tmLanguage syntax highlighting or the rule primitive.

Decision Log:
- Shared stage label: `RULE###` codes reuse the existing `stage="compile"` label rather than introducing a new `rule-check` stage class. The public formatter is already code-agnostic, so a distinct stage class would add surface without changing rendered output. The RULE-band visibly carries the dedicated namespace identity; the stage label stays pragmatic. Recorded as an explicit Decision Log entry instead of a silent deviation from the plan's "fallback stage labels include a `rule-check` stage" checklist bullet.
- `requires declare` scope: the assertion walks the scoped agent's full declared ancestor chain looking for a matching `AuthoredSlot*` entry. A scoped agent satisfies the rule when either it declares the slot directly or one of its declared ancestors does; the rule does not require re-declaration at every level. This matches the way Doctrine already treats inherited slot declarations elsewhere.
...
## 2026-04-19 - Normalize `output_shape_selector_stmt` to expr-based `mode` form (soft-deprecation with timebox)

- Context: Phase 5 delivers the §7 skill-binding `mode` statement by reusing the shipped expr-based `mode_stmt` production from review/law/output-shape carrier surfaces. Repo evidence (`doctrine/grammars/doctrine.lark` before this phase) showed that `output_shape_selector_stmt` alone still used the old enum-only form `mode CNAME as name_ref`, while every other `mode` site already used `mode CNAME = expr as name_ref`. Leaving the drift in place would force the new skill-binding path to either duplicate the old form or quietly allow two incompatible `mode` shapes across the language.
- Options: (a) keep the enum-only form as-is for `output_shape_selector_stmt` and accept permanent drift; (b) hard-cutover to the expr-based form in one release (breaking change for any authored selector still using the old form); (c) normalize the grammar to accept both forms during one minor-version cycle, emit a fatal compile diagnostic on the enum-only form so users see the deprecation loudly, and remove the enum-only alternative at the next minor bump.
- Decision: (c). The expr-based form is already the canonical shape across review, law, and skill-binding `mode` sites. Supporting only one form long-term eliminates a genuine inconsistency. The one-minor-cycle window gives downstream authors time to migrate in step with the language version bump (`5.3 -> 5.4`) announced in `docs/VERSIONING.md` and `CHANGELOG.md`.
- E543 as fatal (architectural call): the Doctrine compiler has no warning infrastructure. Diagnostics are either silent or fatal. Rather than invent a warning channel for one soft-deprecation, the soft-deprecation is implemented as a fatal compile error (`E543`). The authored message in `docs/COMPILER_ERRORS.md` names the canonical form so the fix is one edit. The enum-only grammar alternative stays parseable so existing corpora still reach the validator and get the clear `E543` message; the alternative is scheduled for removal from the grammar at the next minor bump.
- Consequences: all shipped `examples/138_output_shape_case_selector/` fixtures migrate to the expr-based form; one existing enum-only fixture is preserved under `examples/144_skill_binding_producer_audit_mode/prompts/invalid_enum_only_selector/` as the E543 proof case. Authors who still use the enum-only form see `E543` with the migration hint. Downstream tooling (`editors/vscode/`) gets the new syntax added; the old form remains highlighted for the deprecation window.
- Timebox: enum-only `output_shape_selector_stmt` form is scheduled for removal at the next Doctrine language minor bump after `5.4` (i.e., `5.5` or the next minor line). The removal PR must delete the grammar alternative, drop `E543`, and delete the `invalid_enum_only_selector/` fixture. `CHANGELOG.md` already records the deprecation under "Deprecated (language 5.3 -> 5.4)" so the removal has a stable anchor.
- Follow-ups: when the removal lands, update `docs/VERSIONING.md` with the breaking-change note (migration path: rewrite each selector to the expr-based form before upgrading) and retire this Decision Log entry by appending a "closed" note rather than deleting it.

## 2026-04-20 - Rule primitive ships with a closed predicate surface and its own `RULE###` diagnostic band

- Context: Phase 7 introduces the declarative top-level `rule` primitive so projects can encode inheritance/binding invariants as compile-time checks. The plan intentionally ships a closed predicate surface (four scope predicates, three assertion predicates) instead of an open boolean/pattern-match expression language, and carves out a `RULE###` diagnostic band so rule-check diagnostics stay visibly distinct from the `E###` semantic/compile codes. The question: does the language need an open expression DSL on day one, and do rule checks need their own diagnostic stage class to stay distinct?
- Options: (a) ship the closed predicate surface plus a dedicated `RULE###` band using the existing `stage="compile"` label and the existing code-agnostic formatter; (b) ship an open expression language (boolean logic, pattern matching, cross-graph queries) on day one, with its own `rule-eval` stage class; (c) ship the closed predicate surface but reuse the `E###` band instead of carving a new namespace.
- Decision: (a). The closed predicate surface covers the audit's motivating cases (role-class inheritance invariants, retired-ancestor forbids, required-slot declares). Shipping that set first, with codes `RULE001`-`RULE099` reserved for future closed-predicate extensions and `RULE100+` reserved for any future open-expression-language evolution, keeps the compiler surface small and gives a clean future expansion path. The `RULE###` band is the visible namespace mark; the stage label stays `stage="compile"` because the public formatter at `doctrine/diagnostics.py` is already code-agnostic and a dedicated stage class would add surface without changing rendered output. That same formatter renders every `RULE###` code identically to every `E###` code today, so authors never see an inconsistent diagnostic shape.
- Consequences: the shipped rule primitive has a frozen-at-ship predicate set, a frozen diagnostic band with five codes, and a documented reservation policy (`RULE006`-`RULE099` closed extensions; `RULE100+` open-expression evolution). The plan's Phase 7 checklist bullet that asked for a distinct `rule-check` stage label is answered by this Decision Log entry as a deliberate simplification — the shipped behavior carries the dedicated namespace without inventing a dedicated stage. `ValidateRulesMixin` runs during `compile_agent_from_unit` with a per-flow-identity cache so multi-agent flows pay rule-evaluation cost once. The canonical example for the primitive is `examples/146_declarative_project_lint_rule/`, which carries one happy-path `render_contract` case plus one `compile_fail` case per shipped `RULE###` code.
- Follow-ups: if a future audit surfaces a real authoring pain that a closed predicate cannot express (for example, "every composer that binds SkillX must also inherit FromY"), propose the minimal new closed predicate and assign it the next `RULE006+` slot rather than reaching for an open expression language immediately. The `RULE100+` band stays reserved for the case where the real answer is an open expression language; that move itself needs its own Decision Log entry and its own phased plan.

## 2026-04-19 - Typed abstract-agent parameters stay narrow; no convergence with skill `host_contract` slots or output-schema fields

- Context: Phase 6 extends `agent_slot_abstract` with an optional type annotation — `abstract <slot_key>: TypedEntity` — so an abstract agent parent can narrow a slot to one declared family (document, schema, table, enum, agent, or workflow). A surface-level read could argue this looks like the `skill host_contract:` slot-typing pattern (where a skill package declares host-owned slots by declared family) or the output-schema field-typing pattern (where a schema field is narrowed by a declared scalar or entity type). The question: do we converge these three surfaces into one shared typing mechanism?
- Options: (a) share one typed-slot surface across abstract agent parameters, skill `host_contract` slots, and output-schema fields, with one shared resolver, one shared validator, one shared error code family; (b) keep the three surfaces architecturally distinct, each with its own resolver, validator, and error codes, and document the non-convergence so a future contributor does not try to force it back together; (c) share only the resolver internals (the "does this name_ref resolve to an entity of family X?" helper) but keep the surface, validator, and errors separate.
- Decision: (b), with selective code reuse from (c) where it does not bleed semantic distinctions. The three surfaces are semantically distinct: abstract agent parameters narrow which concrete child can bind; `host_contract` slots declare what slots a host must bind (with ownership inversion — the skill names the shape, the host fills it); output-schema fields declare the shape of emitted data (a runtime contract, not an authoring-time binding rule). Forced convergence would blur ownership (authoring-time vs. runtime, skill-owner vs. host-owner), and the `as TypedEntity` annotation on abstract slots does not map cleanly onto the other two surfaces without either widening the abstract-slot semantics (e.g., making them declare ownership inversion) or narrowing the other two. The shipped Phase 6 validator (`_validate_typed_abstract_slot_binding`) and diagnostics (`E538`, `E539`) live in `doctrine/_compiler/validate/agents.py:ValidateAgentsMixin`, independent of the skill-host-slot and output-schema validators. The shared helper `_resolve_typed_slot_family` in `doctrine/_compiler/resolve/agent_slots.py` is scoped to agent-slot resolution only; if a future phase wants to share the entity-family lookup across surfaces, that is a small, safe refactor — the semantic boundaries stay.
- Consequences: three typed-slot surfaces remain in the language (abstract agent parameters, skill `host_contract` slots, output-schema fields). Each keeps its own error code family. The audit on §6 that asked "should these converge?" is answered "no, by design" with this entry as the durable record. Downstream contributors who see the three surfaces and ask the same question get the rationale instead of having to re-derive it. Example 145 (`examples/145_abstract_agent_typed_parameters/`) is the canonical proof for abstract-agent typed parameters; the skill-host-slot pattern already has its own proof in example 142 and the output-schema field-typing pattern already has its own proof across the typed-gate examples.
- Follow-ups: if a future language gap audit surfaces a concrete authoring pain that three separate surfaces cause (for example, a single feature that would genuinely benefit from carrying the same typed-slot shape across all three), revisit with a new Decision Log entry rather than silently collapsing the surfaces. Until then, `abstract <slot>: TypedEntity` stays the narrow convergence this phase shipped.
